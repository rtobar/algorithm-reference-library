...
class TestImageDeconvolution(unittest.TestCase):
    ...
    def test_deconvolve_hogbom(self):
        set_parameters(self.ini, {'niter': 10000, 'gain': 0.1, 'algorithm': 'hogbom', 'threshold': 0.01},
                       'deconvolution')
    ...
    def test_deconvolve_msclean(self):
        set_parameters(self.ini, {'niter': 1000, 'gain': 0.7, 'algorithm': 'msclean', 'scales': [0, 30, 10, 30],
                                  'threshold': 0.01}, 'deconvolution')
    ...
    def test_deconvolve_msclean_1scale(self):
        set_parameters(self.ini, {'niter': 1000, 'gain': 0.7, 'algorithm': 'msclean', 'scales': [0],
                                  'threshold': 0.01}, 'deconvolution')
    ...
    def test_deconvolve_msclean_inner_quarter(self):
        set_parameters(self.ini, {'niter': 1000, 'gain': 0.7, 'algorithm': 'msclean', 'scales': [0],
                                  'threshold': 0.01, 'window': 'quarter'}, 'deconvolution')
        self.comp, self.residual = deconvolve_cube(self.dirty, self.psf, self.ini)
    ...
    def test_deconvolve_hogbom_subpsf(self):
        set_parameters(self.ini, {'niter': 10000, 'gain': 0.1, 'algorithm': 'hogbom',
                                  'threshold': 0.01, 'window': 'quarter', 'psf_support': 200}, 'deconvolution')
        self.comp, self.residual = deconvolve_cube(self.dirty, self.psf, self.ini)
    ...
    def test_deconvolve_msclean_subpsf(self):
        set_parameters(self.ini, {'niter': 1000, 'gain': 0.7, 'algorithm': 'msclean', 'scales': [0, 3, 10, 30],
                                  'threshold': 0.01, 'window': 'quarter', 'psf_support': 200}, 'deconvolution')
        self.comp, self.residual = deconvolve_cube(self.dirty, self.psf, self.ini)
        export_image_to_fits(self.comp, "%s/test_deconvolve_msclean_subpsf-comp.fits" % (self.dir))
        export_image_to_fits(self.residual, "%s/test_deconvolve_msclean_subpsf-residual.fits" % (self.dir))
